...
import AddAccountSlideOver from './AddAccountSlideOver.vue'
import OriginBankModal from './OriginBankModal.vue'
import SourceFundsModal from './SourceFundsModal.vue'
...
// Enviar datos al padre y avanzar
const handleContinue = () => {
  emit('continue', {
    bank: selectedOriginBank.value,
    account: selectedDestinationAccount.value,
    fund: selectedSourceFund.value
  })
}
</script>
...
<template>
  <main class="flex-1 flex flex-col items-center py-6 md:py-10 px-4">
    <h1 class="text-2xl font-bold text-[#060f26] mb-8 text-center hidden md:block">Completa los datos de tu operación</h1>
    ...
    <div class="w-full max-w-[400px] flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-5 mt-2">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <button @click="isOriginModalOpen = true" class="w-full border border-gray-200 rounded-lg p-3 text-sm bg-white flex justify-between items-center hover:border-kambista-cyan transition-colors">
            <div class="flex items-center gap-3">
              <img v-if="selectedOriginBank" :src="`https://cdn.kambista.com/app/images/banks/${selectedOriginBank.alias.toLowerCase()}.png`" class="w-6 h-6 object-contain rounded-sm" />
        ...
        <div class="flex flex-col gap-1.5 relative">
          ...
          <button @click="isSelectAccountModalOpen = true" class="w-full border border-gray-200 rounded-lg p-3 text-sm outline-none bg-white flex justify-between items-center hover:border-kambista-cyan transition-colors">
          ...
          <div class="bg-[#fff7ea] rounded-xl p-3 flex gap-2 items-start mt-1 relative">
             ...
             <p class="text-[11px] text-[#b46b1a] leading-tight pr-4">Recuerda que las cuentas deben <span class="font-bold">estar a tu nombre.</span> Kambista <span class="font-bold">no transfiere a cuentas de terceros.</span></p>
             <button class="absolute top-3 right-3 text-[#b46b1a] hover:opacity-70"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
        </div>

        <div class="flex flex-col gap-1.5">
          <label class="text-[13px] text-gray-500">Origen de fondos</label>
          <button @click="isSourceFundsModalOpen = true" class="w-full border border-gray-200 rounded-lg p-3 text-sm bg-white flex justify-between items-center hover:border-kambista-cyan transition-colors">
            <span :class="selectedSourceFund ? 'text-gray-800' : 'text-gray-400'">{{ selectedSourceFund || 'Selecciona' }}</span>
            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
          </button>
        </div>
    ...
    <OriginBankModal :isOpen="isOriginModalOpen" @close="isOriginModalOpen = false" @select="handleSelectOriginBank" />
    <SourceFundsModal :isOpen="isSourceFundsModalOpen" @close="isSourceFundsModalOpen = false" @select="handleSelectSourceFund" />
    ...
    <div v-if="isSelectAccountModalOpen" class="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all z-10 relative flex flex-col max-h-[80vh]">
        <div class="bg-[#060f26] p-6 text-white shrink-0">
          <button @click="isSelectAccountModalOpen = false" class="absolute top-4 right-4 text-gray-300 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
        ...
        <div class="p-6 overflow-y-auto">
          <div v-if="savedAccounts.length > 0" class="flex flex-col mb-2">
            <div v-for="(acc, index) in savedAccounts" :key="index" @click="selectExistingAccount(acc)" class="border-b border-gray-100 py-4 cursor-pointer hover:bg-gray-50 transition-colors group" :class="selectedDestinationAccount === acc ? 'bg-gray-50/50' : ''">
          ...
          <button @click="openAddAccountSlideOver" class="flex items-center gap-4 py-4 w-full group">
            <div class="w-10 h-10 rounded-md border border-[#060f26] flex items-center justify-center group-hover:border-kambista-cyan group-hover:text-kambista-cyan transition-colors"><svg class="w-5 h-5 text-[#060f26] group-hover:text-kambista-cyan transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg></div>
    ...
    <AddAccountSlideOver :isOpen="isAddAccountOpen" @close="isAddAccountOpen = false" @addAccount="handleAddAccount" />
  </main>
</template>
...
const receiveAmount = ref('37000.00')
</script>
...
<template>
  <div class="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden">
    ...
    <div class="flex w-full h-12 cursor-pointer text-sm font-semibold">
      <div @click="activeTab = 'compra'" class="flex-1 flex items-center justify-center transition-colors" :class="activeTab === 'compra' ? 'bg-[#182233] text-white' : 'bg-white text-gray-500 hover:bg-gray-50'">
        Compra: 3.321
      </div>
      <div @click="activeTab = 'venta'" class="flex-1 flex items-center justify-center transition-colors" :class="activeTab === 'venta' ? 'bg-[#182233] text-white' : 'bg-white text-gray-500 hover:bg-gray-50'">
        Venta: 3.321
      </div>
    </div>
    ...
    <div class="p-5 lg:p-6 space-y-4">
      ...
      <div class="relative flex flex-col gap-2">
        <div class="flex rounded-lg overflow-hidden border border-transparent focus-within:border-kambista-cyan transition-colors">
          <div class="bg-[#e9ecef] flex-1 py-2 px-4 flex flex-col justify-center">
            ...
            <input v-model="sendAmount" type="text" class="bg-transparent text-lg font-bold text-gray-800 outline-none w-full" />
          </div>
          <div class="bg-[#182233] text-white w-1/3 flex items-center justify-between px-4 cursor-pointer">
            <span class="font-semibold text-sm">Dólares</span>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
          </div>
        </div>

        <button class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-full p-1 shadow-md border border-gray-100 hover:bg-gray-50 z-10 transition-colors">
        ...
        <div class="flex rounded-lg overflow-hidden border border-transparent focus-within:border-kambista-cyan transition-colors">
          <div class="bg-[#e9ecef] flex-1 py-2 px-4 flex flex-col justify-center">
            ...
            <input v-model="receiveAmount" type="text" class="bg-transparent text-lg font-bold text-gray-800 outline-none w-full" />
          </div>
          <div class="bg-[#182233] text-white w-1/3 flex items-center justify-between px-4 cursor-pointer">
            <span class="font-semibold text-sm">Soles</span>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-between items-center text-sm">
        <div>
          ...
          <p class="font-bold text-gray-800 text-left">S/ 555.00</p>
        </div>
        <div class="text-right">
          ...
          <p class="font-bold text-gray-800 flex items-center gap-1 justify-end">
            ...
            10,000 <span class="text-yellow-400">🪙</span>
          </p>
        </div>
      </div>
